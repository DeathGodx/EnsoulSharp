...
namespace hi_im_gosu
{
    public class Vayne
    {
        ...
        public static void Orbwalking_AfterAttack(Object targeti ,OrbwalkerActionArgs args)
        {
            ...
            if ((Orbwalker.ActiveMode.HasFlag(OrbwalkerMode.LaneClear) || Orbwalker.ActiveMode.HasFlag(OrbwalkerMode.LaneClear)) && 100 * (Player.Mana / Player.MaxMana) > getMenuSliderItem(qmenu, "Junglemana"))
            {
                var mob = MinionManager.GetMinions(Player.Position, E.Range).FirstOrDefault();
                var Minions = MinionManager.GetMinions(Player.Position.Extend(Game.CursorPosRaw, Q.Range), Player.AttackRange);
                ...
                if (countMinions >= 2 && useQ && Q.IsReady() && Minions != null)
                    Q.Cast(Player.Position.Extend(Game.CursorPosRaw, Q.Range / 2));
            ...
            if (Q.IsReady() && ((Orbwalker.ActiveMode.HasFlag(OrbwalkerMode.Combo) && getMenuBoolItem(qmenu, "UseQC")) || (Orbwalker.ActiveMode.HasFlag(OrbwalkerMode.Harass) && getMenuBoolItem(qmenu, "hq"))))
            {
                if (getMenuBoolItem(qmenu, "restrictq"))
                {
                    var after = ObjectManager.Player.Position + Normalize(Game.CursorPosRaw - ObjectManager.Player.Position) * 300;
        ...
        {
            if (getMenuBoolItem(menu, "useR") && R.IsReady() && ObjectManager.Player.CountEnemyHeroesInRange(1000) >= getMenuSliderItem(menu, "enemys") && Orbwalker.ActiveMode.HasFlag(OrbwalkerMode.Combo))
            {
                R.Cast();
            }
            ...
            if (!E.IsReady()) return;
            if ((Orbwalker.ActiveMode.HasFlag(OrbwalkerMode.Combo) && getMenuBoolItem(emenu, "UseEC")) || (Orbwalker.ActiveMode.HasFlag(OrbwalkerMode.Harass) && getMenuBoolItem(emenu, "he")) || getMenuKeyBindItem(emenu, "UseET"))
            {
