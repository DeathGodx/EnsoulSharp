...
namespace Cassiopeia_Du_Couteau_2
{
    static class Program
    {
        ...
        public static void Loading_OnLoadingComplete()
        {
            ...
            Chat.Print("Cassiopeia Du Couteau by Horizon - Loaded PORTED By DEATHGODX", System.Drawing.Color.Crimson);
        ...
        {
            var MHR = GameObjects.EnemyMinions.Where(a => a.Distance(Player.Instance) <= _Q.Range).OrderBy(a => a.Health).FirstOrDefault();
            if (MHR != null)
            {



                if (ClearMenu["UseQLH"].GetValue<MenuBool>().Enabled && _Q.IsReady() && Player.Instance.ManaPercent > ClearMenu["ClearMana"].GetValue<MenuSlider>().Value && MHR.IsValidTarget(_Q.Range) &&
                    Player.Instance.GetSpellDamage(MHR, SpellSlot.Q) >= MHR.Health)
                ...
                if (ClearMenu["UseWLH"].GetValue<MenuBool>().Enabled && _W.IsReady() && Player.Instance.GetSpellDamage(MHR, SpellSlot.W) >= MHR.Health &&
                    Player.Instance.ManaPercent > ClearMenu["ClearMana"].GetValue<MenuSlider>().Value)
                {
        ...
        {
            var MHR = GameObjects.Jungle.Where(a => a.Distance(Player.Instance) <= _Q.Range).OrderBy(a => a.Health).FirstOrDefault();
            if (MHR != null)
            {
...

        {
            ...
            if (ClearMenu["UseECL"].GetValue<MenuBool>().Enabled && _E.IsReady() && Player.Instance.ManaPercent > ClearMenu["ClearMana"].GetValue<MenuSlider>().Value && MHR.IsValidTarget(_E.Range))
