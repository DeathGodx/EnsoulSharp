...
namespace Diana___Bloody_Lunari
{
...

    {
        ...
        public static void Loading_OnLoadingComplete()
        {
            ...
            Chat.Print("Good luck and have fun, DEATHGODX.", System.Drawing.Color.DarkViolet); /*Color.DarkViolet);*/
        ...
        private static void Game_OnUpdate(EventArgs args)
        {
            ...
            if (Orbwalker.ActiveMode.Equals(OrbwalkerMode.Combo))
            {
                Combo();
            }
        ...
        private static void Combo()
        {
            ...
            {
                if (_E.IsInRange(target) && !Player.Instance.InAutoAttackRange())
                {
                    _E.Cast();
                }
            }
        ...
        {
            var jungleMonsters = GameObjects.EnemyMinions.Where(a => a.Distance(Player.Instance) <= _Q.Range).OrderBy(a => a.Health).FirstOrDefault();
            if (jungleMonsters != null)
            {
                if (LastHitM["Qlh"].GetValue<MenuBool>().Enabled && _Q.IsReady() && Player.Instance.ManaPercent > LastHitM["manalh"].GetValue<MenuSlider>().Value && jungleMonsters.IsValidTarget(_Q.Range) &&
                    Player.Instance.GetSpellDamage(jungleMonsters, SpellSlot.Q) >= jungleMonsters.Health + jungleMonsters.AllShield)

                {
                    _Q.Cast(jungleMonsters);
                }

                if (LastHitM["Elh"].GetValue<MenuBool>().Enabled && _W.IsReady() && Player.Instance.GetSpellDamage(jungleMonsters, SpellSlot.W) >= jungleMonsters.Health + jungleMonsters.AllShield &&
                    Player.Instance.ManaPercent > LastHitM["manalh"].GetValue<MenuSlider>().Value)
                {
                    _W.Cast();
                }
            }
