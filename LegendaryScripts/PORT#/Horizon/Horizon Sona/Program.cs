...
namespace The_Horizon_Sona
{
    class Program
    {
        ...
        {
            return Player.Instance.GetSpellSlot(Name);
        }
        ...
        private static void Loading_OnLoadingComplete()
        {
            ...
            Chat.Print("The Horizon Sona - Loaded ! PORTED By DEATHGODx");
        ...
        public static void FlashR()
        {
            ...
            if (target.IsValidTarget(_FlashR.Range))
                if (!target.IsValidTarget(_FlashR.Range))
                    return;
            {
                var Flashh = Player.Instance.Position.Extend(target.Position, _Flash.Range);
