...
namespace Tristana
{
    static class Program
    {
        ...
        private static void OnLoadingComplete()
        {
            ...
            Chat.Print("Doctor's Tristana Loaded! PORTED by DEATHGODx", Color.Orange);
        ...
        {
            var target = GameObjects.EnemyHeroes.Where(e => e.IsValidTarget(W.Range) && !e.HasBuff("JudicatorIntervention") && !e.HasBuff("kindredrnodeathbuff") && !e.HasBuff("Undying Rage") && !e.IsDead && !e.IsZombie);
            ...
            foreach (var target2 in target)
            {
                ...
                {
                    if (target2.Health + target2.AllShield <= Player.Instance.GetAutoAttackDamage(target2) * WAttack && Player.Instance.HealthPercent >= 10 && target2.Position.CountEnemyHeroesInRange(600) <= minW)
                    {
        ...
        private static void Drawing_OnDraw(EventArgs args)
        {
            ...
            if (Misc["Notifications"].GetValue<MenuBool>().Enabled && R.IsReady())
            {
                ...
                if (target.IsValidTarget(1000) && Player.Instance.GetSpellDamage(target, SpellSlot.R) > target.Health + target.PhysicalShield)
                {
