...
namespace Twitch
{
    internal class Program
    {
        ...
        public static void Combo()
        {
            ...
            foreach (var target in GameObjects.EnemyHeroes.Where(e => e.IsValidTarget(E.Range) && !e.IsDead && !e.IsZombie))
            {
                ...
                if (useW && W.IsReady() && !QCasting && target.IsValidTarget(W.Range) && _Player.Distance(target) > Player.Instance.GetRealAutoAttackRange(target))
                {
        ...
        public static void ResetAttack(object e, OrbwalkerActionArgs args)
        {
            ...
            if (target != null)
            {
                if (useW && W.IsReady() && target.IsValidTarget(W.Range) && _Player.Distance(target) < Player.Instance.GetRealAutoAttackRange(target) && Orbwalker.ActiveMode.HasFlag(OrbwalkerMode.Combo))
                {
                    var Pred = W.GetPrediction(target);
        ...
        public static void Item()
        {
            ...
            foreach (var target in GameObjects.EnemyHeroes.Where(e => e.IsValidTarget(900) && !e.IsDead))
            {
                ...
                if (yous && Youmuu.IsReady && Youmuu.IsOwned() && _Player.Distance(target) <= Player.Instance.GetRealAutoAttackRange() && Orbwalker.ActiveMode.HasFlag(OrbwalkerMode.Combo))
                {
        ...
        private static void Orbwalker_CantLasthit(object targeti, OrbwalkerActionArgs args)
        {
            ...
            var unit = (useE && Orbwalker.ActiveMode.HasFlag(OrbwalkerMode.LaneClear) && Player.Instance.ManaPercent >= mana);
