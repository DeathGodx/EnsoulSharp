...
namespace Olaf7
{
    internal class Program
    {
        ...
        static void OnLoadingComplete()
        {
            ...
            Chat.Print("Doctor's Olaf Loaded! PORTED by DEATHGODx", Color.Orange);
        ...
        public static void LastHit()
        {
            ...
            if (minion != null)
            {
                if (useQ && Q.IsReady() && Player.Instance.ManaPercent > LhM && minion.IsValidTarget(Q.Range) && Player.Instance.GetSpellDamage(minion, SpellSlot.Q) >= minion.Health + minion.AllShield)
                {
                    Q.Cast(minion);
                }

                if (useE && E.IsReady() && Player.Instance.GetSpellDamage(minion, SpellSlot.E) >= minion.Health + minion.AllShield)
                {
                    E.Cast(minion);
                }
            }
        ...
        private static void Ult()
        {
            ...
            var cc = (Misc["silence"].GetValue<MenuBool>().Enabled && Player.HasBuffOfType(BuffType.Silence))
            || (Misc["snare"].GetValue<MenuBool>().Enabled && Player.HasBuffOfType(BuffType.Snare))
            || (Misc["supperss"].GetValue<MenuBool>().Enabled && Player.HasBuffOfType(BuffType.Suppression))
            || (Misc["sleep"].GetValue<MenuBool>().Enabled && Player.HasBuffOfType(BuffType.Sleep))
            || (Misc["poly"].GetValue<MenuBool>().Enabled && Player.HasBuffOfType(BuffType.Polymorph))
            || (Misc["frenzy"].GetValue<MenuBool>().Enabled && Player.HasBuffOfType(BuffType.Frenzy))
            || (Misc["disarm"].GetValue<MenuBool>().Enabled && Player.HasBuffOfType(BuffType.Disarm))
            || (Misc["nearsight"].GetValue<MenuBool>().Enabled && Player.HasBuffOfType(BuffType.NearSight))
            || (Misc["knockback"].GetValue<MenuBool>().Enabled && Player.HasBuffOfType(BuffType.Knockback))
            || (Misc["knockup"].GetValue<MenuBool>().Enabled && Player.HasBuffOfType(BuffType.Knockup))
            || (Misc["slow"].GetValue<MenuBool>().Enabled && Player.HasBuffOfType(BuffType.Slow))
            || (Misc["poison"].GetValue<MenuBool>().Enabled && Player.HasBuffOfType(BuffType.Poison))
            || (Misc["blind"].GetValue<MenuBool>().Enabled && Player.HasBuffOfType(BuffType.Blind))
            || (Misc["charm"].GetValue<MenuBool>().Enabled && Player.HasBuffOfType(BuffType.Charm))
            || (Misc["tunt"].GetValue<MenuBool>().Enabled && Player.HasBuffOfType(BuffType.Taunt))
            || (Misc["stun"].GetValue<MenuBool>().Enabled && Player.HasBuffOfType(BuffType.Stun))
            || (Misc["rot"].GetValue<MenuBool>().Enabled)
            || (Misc["fear"].GetValue<MenuBool>().Enabled && Player.HasBuffOfType(BuffType.Fear));
            if (R.IsReady() && ulti && cc && _Player.Position.CountEnemyHeroesInRange(800) >= minR && Player.Instance.HealthPercent <= heal)
            {
                R.Cast();
            }
        }
