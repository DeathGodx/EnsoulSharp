...
namespace Renekton7
{
    internal class Program
    {
        ...
        static void OnLoadingComplete()
        {
            ...
            Chat.Print("Doctor's Renekton Loaded! PORTED by DEATHGODx", Color.Orange);
        ...
        private static void Ultimate()
        {
            ...
            if (useR && _Player.HealthPercent <= minR && _Player.Position.CountEnemyHeroesInRange(500) >= 1 && !Player.Instance.InShop())
            {
                R.Cast();
            }

            if (useR2 && !Player.Instance.InShop() && _Player.Position.CountEnemyHeroesInRange(450) >= minE)
            {
                R.Cast();
            }
        }
        ...
        private static void KillSteal()
        {
            ...
            foreach (var target in GameObjects.EnemyHeroes.Where(hero => hero.IsValidTarget(E.Range) && !hero.HasBuff("JudicatorIntervention") && !hero.HasBuff("kindredrnodeathbuff") && !hero.HasBuff("Undying Rage") && !hero.IsDead && !hero.IsZombie))
            {
