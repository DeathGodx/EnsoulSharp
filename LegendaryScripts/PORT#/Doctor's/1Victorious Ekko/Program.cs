...
namespace Ekko
{
    static class Program
    {
        ...
        private static void Game_OnUpdate(EventArgs args)
        {
            ...
            if (Orbwalker.ActiveMode.HasFlag(OrbwalkerMode.None))
            {
                Flee();
            }
        ...
        private static void Ultimate()
        {
            ...
            var enemies = GameObjects.EnemyHeroes.Where(e => e.IsValidTarget() && !e.IsZombie).ToArray();
        ...
        {
            var Enemies = GameObjects.EnemyHeroes.Where(e => e.IsValidTarget(Q.Range) && !e.HasBuff("JudicatorIntervention") && !e.HasBuff("kindredrnodeathbuff") && !e.HasBuff("Undying Rage") && !e.IsDead && !e.IsZombie);
        ...
        private static void AIHeroClient_OnProcessSpellCast(AIBaseClient sender, AIBaseClientProcessSpellCastEventArgs args)
        {
            ...
            if (useREs && R.IsReady())
            {
                ...
                if (sender.BaseAttackDamage >= Player.Instance.Health + Player.Instance.AllShield || sender.BaseAbilityDamage >= Player.Instance.Health + Player.Instance.AllShield)
                {
                    R.Cast();
                }

                if (sender.GetAutoAttackDamage(Player.Instance) >= Player.Instance.Health + Player.Instance.AllShield)
                {
                    R.Cast();
                }
            }
        ...
        public static void Miscs()
        {
            ...
            var Enemies = GameObjects.EnemyHeroes.Where(e => e.IsValidTarget(Q.Range) && e.GetBuffCount("EkkoStacks") == 2 && !e.HasBuff("JudicatorIntervention") && !e.HasBuff("kindredrnodeathbuff") && !e.HasBuff("Undying Rage") && !e.IsDead && !e.IsZombie);
