...
namespace KogMaw
{
    internal class Program
    {
        ...
        public static void ResetAttack(object e, OrbwalkerActionArgs args)
        {
            ...
            if (target != null)
            {
                if (useE && E.IsReady() && target.IsValidTarget(E.Range) && Orbwalker.ActiveMode.HasFlag(OrbwalkerMode.Combo))
                {
        ...
        private static void LaneClear()
        {
            ...
            {
                if (useQ && Q.IsReady() && minions.IsValidTarget(Q.Range) && Player.Instance.GetSpellDamage(minions, SpellSlot.Q) > minions.Health + minions.AllShield && _Player.Distance(minions) > 500)
                {
                    Q.Cast(minions);
                }

                if (useR && R.IsReady() && minions.IsValidTarget(R.Range) && Player.Instance.GetBuffCount("kogmawlivingartillerycost") <= Rlimit)
                {
                    R.Cast(minions);
                }
        ...
        public static void JungleClear()
        {
            ...
            if (monster != null)
            {
                ...
                if (useR && R.IsReady() && Player.Instance.GetBuffCount("kogmawlivingartillerycost") < Rlimit)
                {
                    R.Cast(monster);
                }
            }
