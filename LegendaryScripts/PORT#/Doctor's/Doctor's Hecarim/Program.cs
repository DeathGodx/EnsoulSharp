...
namespace Hecarim7
{
    internal class Program
    {
        ...
        static void OnLoadingComplete()
        {
            ...
            Chat.Print("Doctor's Hecarim Loaded! Ported by DEATHGODx", Color.Orange);
        ...
        private static void LaneClear()
        {
            ...
            var minion = GameObjects.EnemyMinions.Where(a => a.Distance(Player.Instance) <= W.Range).OrderBy(a => a.Health).FirstOrDefault();
            if (minion != null)
            {
                ...
                if (useWLH && W.IsReady() && minion.IsValidTarget(W.Range) && Player.Instance.ManaPercent >= laneWMN)
                {
                    W.Cast();
                }
            }
        ...
        private static void AutoQ()
        {
            ...
            {
                if (useQ && Q.IsReady() && !Tru(_Player.Position) && Player.Instance.ManaPercent >= mana && target.IsValidTarget(Q.Range) && !Orbwalker.ActiveMode.HasFlag(OrbwalkerMode.Combo) && !Orbwalker.ActiveMode.HasFlag(OrbwalkerMode.Harass))
                {
                    Q.Cast();
                }
            }
        ...
        private static void KillSteal()
        {
            ...
            foreach (var target in GameObjects.EnemyHeroes.Where(hero => hero.IsValidTarget(R.Range) && !hero.HasBuff("JudicatorIntervention") && !hero.HasBuff("kindredrnodeathbuff") && !hero.HasBuff("Undying Rage") && !hero.IsDead && !hero.IsZombie))
            {
                ...
                {
                    if (target.Health + target.AllShield <= Player.Instance.GetSpellDamage(target, SpellSlot.W))
                    {
                        W.Cast();
                    }
                }
                ...
                {
                    if (target.Health + target.AllShield <= RDamage(target) && (!target.IsValidTarget(minKsR) || Player.Instance.HealthPercent <= 25))
                    {
                        var RPred = R.GetPrediction(target);
                        if (RPred.Hitchance >= HitChance.High)
                        {
