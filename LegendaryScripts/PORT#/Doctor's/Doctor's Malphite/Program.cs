...
namespace Malphite
{
    internal class Program
    {
        ...
        static void OnLoadingComplete()
        {
            ...
            Chat.Print("Doctor's Malphite Loaded! Ported by DEATGODx", Color.Orange);
            Bootstrap.Init(null);
        ...
        private static void Drawing_OnDraw(EventArgs args)
        {
            ...
            if (Drawings["Notifications"].GetValue<MenuBool>().Enabled && R.IsReady())
            {
                ...
                if (target != null && target.IsValidTarget(R.Range) && Player.Instance.GetSpellDamage(target, SpellSlot.R) > target.Health + target.PhysicalShield)
                {
        ...
        private static void KillSteal()
        {
            ...
            foreach (var target in GameObjects.EnemyHeroes.Where(hero => hero.IsValidTarget(R.Range) && !hero.HasBuff("JudicatorIntervention") && !hero.HasBuff("kindredrnodeathbuff") && !hero.HasBuff("Undying Rage") && !hero.IsDead && !hero.IsZombie))
            {
                ...
                {
                    if (target.Health + target.PhysicalShield <= Player.Instance.GetSpellDamage(target, SpellSlot.E))
                    {
                        E.Cast();
                    }
                }
                ...
                if (KsR && R.IsReady())
                {
                    if (target.Health + target.PhysicalShield <= Player.Instance.GetSpellDamage(target, SpellSlot.R) && !target.IsValidTarget(minKsR))
                    {
                        R.Cast(target);
                    }
                }
