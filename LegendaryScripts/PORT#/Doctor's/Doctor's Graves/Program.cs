    ...
    internal static class Program
    {
        ...
        private static void Drawing_OnDraw(EventArgs args)
        {
            ...
            if (Drawings["Notifications"].GetValue<MenuBool>().Enabled && R.IsReady())
            {
                ...
                if (target.IsValidTarget(R.Range) && Player.Instance.GetSpellDamage(target, SpellSlot.R) > target.Health + target.PhysicalShield)
                {
        ...
        public static void Item()
        {
            ...
            foreach (var target in GameObjects.EnemyHeroes.Where(e => e.IsValidTarget(550) && !e.IsDead))
            {
                ...
                if (yous && Youmuu.IsReady && Youmuu.IsOwned() && target.IsValidTarget(530) && Orbwalker.ActiveMode.HasFlag(OrbwalkerMode.Combo))
                {
                    Youmuu.Cast();
                }
            }
        ...
        private static void ResetAttack(object target, OrbwalkerActionArgs args)
        {
            ...
            if (useJ && E.IsReady() && Orbwalker.ActiveMode.HasFlag(OrbwalkerMode.LaneClear) && Player.Instance.ManaPercent >= manaJ)
            {
                Player.CastSpell(SpellSlot.E, Game.CursorPosCenter);
                Orbwalker.ResetAutoAttackTimer();
            }

            if (useL && E.IsReady() && Orbwalker.ActiveMode.HasFlag(OrbwalkerMode.LaneClear) && Player.Instance.ManaPercent >= mana)
            {
                Player.CastSpell(SpellSlot.E, Game.CursorPosCenter);
                Orbwalker.ResetAutoAttackTimer();
            }

            if (HarassMenu["HarassAA"].GetValue<MenuBool>().Enabled && E.IsReady() && Orbwalker.ActiveMode.HasFlag(OrbwalkerMode.Harass) && Player.Instance.ManaPercent >= HarassMenu["ManaHarass"].GetValue<MenuSlider>().Value)
            {
                Player.CastSpell(SpellSlot.E, Game.CursorPosCenter);
                Orbwalker.ResetAutoAttackTimer();
            }
        ...
        public static void Combo()
        {
            ...
            foreach (var target in GameObjects.EnemyHeroes.Where(e => e.IsValidTarget(R.Range) && !e.IsDead && !e.IsZombie))
            {
                ...
                if (useE && E.IsReady())
                {
                    if (ComboMode.Value == 1 && !Player.HasBuff("GravesBasicAttackAmmo2") && _Player.Position.CountEnemyHeroesInRange(R.Range) >= 1)
                    {
                        Player.CastSpell(SpellSlot.E, Game.CursorPosCenter);
                    }
                    //Olmazsa CursorPosCenteri düzelt Sil)
                    if (ComboMode.Value == 0 && !Player.HasBuff("GravesBasicAttackAmmo2") && !Player.HasBuff("GravesBasicAttackAmmo1") && _Player.Position.CountEnemyHeroesInRange(R.Range) >= 1)
                    {
                        Player.CastSpell(SpellSlot.E, Game.CursorPosCenter);
                    }
                }
        ...
        private static void Harass()
        {
            ...
            foreach (var target in GameObjects.EnemyHeroes.Where(e => e.IsValidTarget(W.Range) && !e.IsDead && !e.IsZombie))
            {
        ...
        private static void LaneClear()
        {
            ...
            {
                if (useQ && Q.IsReady() && minion.IsValidTarget(Q.Range) && Player.Instance.ManaPercent > ManaQ && quang.MinionsHit >= MinQ)
                {
        ...
        private static void KillSteal()
        {
            ...
            foreach (var target in GameObjects.EnemyHeroes.Where(e => e.IsValidTarget(R.Range) && !e.HasBuff("BlitzcrankManaBarrierCD") && !e.HasBuff("JudicatorIntervention") && !e.HasBuff("kindredrnodeathbuff") && !e.HasBuff("Undying Rage") && !e.IsDead && !e.IsZombie))
            {
                ...
                if (KsR && R.IsReady())
                {
                    if (target.Health + target.PhysicalShield < Player.Instance.GetSpellDamage(target, SpellSlot.R) && target.IsValidTarget(R.Range) && !target.IsValidTarget(minKsR))
                    {
                        var pred = R.GetPrediction(target);
