...
namespace Vladimir
{
    class Program
    {
        ...
        private static void OnLoadingComplete()
        {
            ...
            Chat.Print("Doctor's Vladimir Loaded! PORTED by DeathGODX", Color.Orange);
            ...
            foreach (var enemies in GameObjects.EnemyHeroes.Where(a => a.Team != Player.Instance.Team))
            {
        ...
        public static void AutoQ()
        {
            ...
            if (!Orbwalker.ActiveMode.HasFlag(OrbwalkerMode.Combo)
            && !Orbwalker.ActiveMode.HasFlag(OrbwalkerMode.Harass)
            && !Orbwalker.ActiveMode.HasFlag(OrbwalkerMode.LaneClear)
            && !Orbwalker.ActiveMode.HasFlag(OrbwalkerMode.LastHit))
            {
        ...
        private static void AIHeroClient_OnProcessSpellCast(AIBaseClient sender, AIBaseClientProcessSpellCastEventArgs args)
        {
            if ((args.Slot == SpellSlot.Q || args.Slot == SpellSlot.W || args.Slot == SpellSlot.E ||
                 args.Slot == SpellSlot.R) && sender.IsEnemy && W.IsReady() && _Player.Distance(sender) <= args.SData.CastRange && Orbwalker.ActiveMode.HasFlag(OrbwalkerMode.Combo))
            {
