    ...
    static class Program
    {
        ...
        private static void Ultimate()
        {
            ...
            if (target != null)
            {
                ...
                if (useR2 && R.IsReady() && !RActive && _Player.HealthPercent <= mauR && _Player.Position.CountEnemyHeroesInRange(R.Range) >= 1 && !Player.Instance.InShop())
                {
                    R.Cast();
                }

                if (autow && W.IsReady() && !RActive && _Player.HealthPercent <= mauW && _Player.Position.CountEnemyHeroesInRange(R.Range) >= 1 && !Player.Instance.InShop())
                {
                    W.Cast();
                }
        ...
        private static void LaneClear()
        {
            ...
            foreach (var minion in minions)
            {
                ...
                && Player.Instance.Distance(minion.Position) <= 300
        ...
        public static void Flee()
        {
            if (E.IsReady())
            {
                var CursorPos = Game.CursorPos2D;
        ...
        private static void KillSteal()
        {
            ...
            foreach (var target in GameObjects.EnemyHeroes.Where(hero => hero.IsValidTarget(E.Range) && !hero.HasBuff("BlitzcrankManaBarrierCD") && !hero.HasBuff("JudicatorIntervention") && !hero.HasBuff("kindredrnodeathbuff") && !hero.HasBuff("Undying Rage") && !hero.IsDead && !hero.IsZombie))
            {
                if (KsE && E.IsReady() && !RActive && target.IsValidTarget(E.Range))
                {
                    if (target.Health + target.Health + target.AllShield < Player.Instance.GetSpellDamage(target, SpellSlot.E) + Player.Instance.GetSpellDamage(target, SpellSlot.Q))
                    {
                        E.Cast(target);
                    }
                }
                ...
                {
                    if (target.Health + target.Health + target.AllShield < Player.Instance.GetSpellDamage(target, SpellSlot.Q))
                    {
                        Q.Cast();
                    }
                }
