...
namespace Kassadin7
{
    internal class Program
    {
        ...
        static void OnLoadingComplete()
        {
            ...
            Chat.Print("Doctor's Kassadin Loaded! PORT by DEATHGODX", Color.Orange);
            Bootstrap.Init(null);
        ...
        private static void Combo()
        {
            ...
            if (target != null)
            {
                ...
                {
                    if (_Player.Distance(target) < Player.Instance.GetRealAutoAttackRange(target) && !W.IsReady())
                    {
                        Q.Cast(target);
                    }
                    else if (_Player.Distance(target) > Player.Instance.GetRealAutoAttackRange(target))
                    {
                        Q.Cast(target);
                    }
                }
        ...
        private static void Harass()
        {
            ...
            if (target != null)
            {
                ...
                if (useR && R.IsReady() && target.IsValidTarget(R.Range) && target.Position.CountEnemyHeroesInRange(R.Range) <= minR && _Player.HealthPercent >= Minhp && Player.Instance.GetBuffCount("RiftWalk") < minRs)
                {
                    if (turret)
                    {
                        if (!UnderTuret(target))
