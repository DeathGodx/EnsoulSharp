    ...
    static class Program
    {
        ...
        private static void Combo()
        {
            ...
            if (target != null)
            {
                ...
                {
                    if (_Player.Distance(target) <= target.GetRealAutoAttackRange() && Player.Instance.HealthPercent <= 60)
                    {
                        W.Cast();
                    }
        ...
        private static void Ultimate()
        {
            ...
            foreach (var target in GameObjects.EnemyHeroes.Where(e => e.IsValidTarget(E.Range) && !e.IsDead))
            {
                ...
                {
                    if (useR && R.IsReady() && !Player.Instance.InShop() && (target.IsValidTarget(E.Range) || _Player.IsUnderEnemyTurret()))
                    {
        ...
        {
            E.Cast(fly.Position.Extend(Game.CursorPosRaw, E.Range).ToVector2());
            /*if (E.IsReady())
            {
