...
namespace Jax
{
    internal class Program
    {
        ...
        static void OnLoadingComplete()
        {
            ...
            Chat.Print("Doctor's Jax Loaded! PORTED by DEATHGODx", Color.White);
        ...
        public static void Item()
        {
            ...
            if (useR && R.IsReady() && _Player.Position.CountEnemyHeroesInRange(Q.Range) >= 1 && Player.Instance.HealthPercent <= MinR)
            {
                R.Cast();
            }
        ...
        public static void Combo()
        {
            ...
            if (ComboMenu["comboMode"].GetValue<MenuList>().Index == 0)
            {
                if (target != null)
                {
                    ...
                    if (useQ && Q.IsReady() && target.IsValidTarget(Q.Range) && (Player.Instance.GetRealAutoAttackRange() < target.Distance(Player.Instance) || Player.Instance.HealthPercent <= 25))
                    {
                        if (ECasting)
                        {
                            Q.Cast(target);
            ...
            if (ComboMenu["comboMode"].GetValue<MenuList>().Index == 1)
            {
                if (target != null)
                {
                    if (useQ && Q.IsReady() && Player.Instance.GetRealAutoAttackRange() < target.Distance(Player.Instance))
                    {
        ...
        public static void LaneClear()
        {
            ...
            {
                if (useQ && Q.IsReady() && Q.IsInRange(minion) && _Player.GetRealAutoAttackRange() <= minion.Distance(Player.Instance) && Player.Instance.GetSpellDamage(minion, SpellSlot.Q) >= minion.Health + minion.AllShield)
                {
                    Q.Cast(minion);
                }
                ...
                && Player.Instance.Distance(minion.Position) <= 225f
        ...
        public static void LastHit()
        {
            ...
            foreach (var minion in minions)
            {
                if (useQ && Q.IsReady() && Q.IsInRange(minion) && Player.Instance.GetRealAutoAttackRange() <= minion.Distance(Player.Instance) && Player.Instance.GetSpellDamage(minion, SpellSlot.Q) > minion.Health + minion.AllShield)
                {
                    Q.Cast(minion);
                }
        ...
        {
            var jumpPos = Game.CursorPosCenter;
        ...
        public static void KillSteal()
        {
            ...
            foreach (var target in GameObjects.EnemyHeroes.Where(hero => hero.IsValidTarget(Q.Range) && !hero.HasBuff("JudicatorIntervention") && !hero.HasBuff("kindredrnodeathbuff") && !hero.IsDead && !hero.IsZombie))
            {
                ...
                {
                    if (target.Health <= _Player.GetSummonerSpellDamage(target, SummonerSpell.Ignite) && (Player.Instance.GetAutoAttackDamage(target) < target.Health || !target.IsValidTarget(Player.Instance.GetRealAutoAttackRange())))
                    {
                        Ignite.Cast(target);
                    }
                }
