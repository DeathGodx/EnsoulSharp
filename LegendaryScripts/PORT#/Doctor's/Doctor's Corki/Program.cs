    ...
    static class Program
    {
        ...
        private static void ResetAttack(object e, OrbwalkerActionArgs args)
        {
            ...
            if (target != null)
            {
                ...
                {
                    if (useQ && Q.IsReady() && target.IsValidTarget(Q.Range) && _Player.Distance(target) < Player.Instance.GetRealAutoAttackRange(target) && Orbwalker.ActiveMode.HasFlag(OrbwalkerMode.Combo))
                    {
                ...
                {
                    if (useR && R.IsReady() && R.Ammo >= 1 && target.IsValidTarget(R.Range) && _Player.Distance(target) < Player.Instance.GetRealAutoAttackRange(target) && Orbwalker.ActiveMode.HasFlag(OrbwalkerMode.Combo))
                    {
        ...
        private static void Combo()
        {
            ...
            if (target != null)
            {
                if (useQ && Q.IsReady() && target.IsValidTarget(Q.Range) && !Orbwalker.CanAttack())
                {
                    ...
                    {
                        if (_Player.Distance(target) > Player.Instance.GetRealAutoAttackRange(target))
                        {
                            var Pred = Q.GetPrediction(target);
                            if (Pred.Hitchance >= HitChance.Medium)
                            {
        ...
        private static void KillSteal()
        {
            ...
            foreach (var target in GameObjects.EnemyHeroes.Where(e => e.IsValidTarget(R.Range) && !e.HasBuff("JudicatorIntervention") && !e.HasBuff("kindredrnodeathbuff") && !e.HasBuff("Undying Rage") && !e.IsDead && !e.IsZombie))
            {
