    ...
    internal class Program
    {
        ...
        public static void ResetAttack(object e, OrbwalkerActionArgs args)
        {
            ...
            if (target != null)
            {
                ...
                {
                    if (Orbwalker.ActiveMode.HasFlag(OrbwalkerMode.Combo) && Q.IsReady())
                    {
                        Q.Cast();
                ...
                if (HasQ && Q.IsReady() && Orbwalker.ActiveMode.HasFlag(OrbwalkerMode.Harass) && Player.Instance.ManaPercent >= Hmana)
                {
                    Q.Cast();
        ...
        private static void KillSteal()
        {
            ...
            foreach (var target in GameObjects.EnemyHeroes.Where(hero => hero.IsValidTarget(475) && !hero.HasBuff("JudicatorIntervention") && !hero.HasBuff("kindredrnodeathbuff") && !hero.HasBuff("Undying Rage") && !hero.IsDead && !hero.IsZombie))
            {
