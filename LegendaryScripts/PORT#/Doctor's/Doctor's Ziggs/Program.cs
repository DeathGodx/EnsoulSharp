...
namespace Ziggs7
{
    internal class Program
    {
        ...
        static void OnLoadingComplete()
        {
            ...
            Chat.Print("Doctor's Ziggs Loaded! PORTED By DEATHGODx", Color.Orange);
            Bootstrap.Init(null);
            Q = new Spell(SpellSlot.Q, 850);
            Q.SetSkillshot(300, 1700, 130, false, SkillshotType.Circle);
            Q2 = new Spell(SpellSlot.Q, 1125);
            Q2.SetSkillshot(300 + Q.Delay, 1700, 130, false, SkillshotType.Line);
            Q3 = new Spell(SpellSlot.Q, 1400);
            Q3.SetSkillshot(300 + Q.Delay, 1700, 130, false, SkillshotType.Line);
            W = new Spell(SpellSlot.W, 1000);
            W.SetSkillshot(250, 1750, 275, false, SkillshotType.Circle);
            E = new Spell(SpellSlot.E, 900);
            E.SetSkillshot(500, 1600, 90, false, SkillshotType.Circle);
            R = new Spell(SpellSlot.R, 5000);
            R.SetSkillshot(1000, 2800, 500, false, SkillshotType.Circle);
        ...
        private static void KillSteal()
        {
            ...
            foreach (var target in GameObjects.EnemyHeroes.Where(hero => hero.IsValidTarget(Q3.Range) && !hero.HasBuff("JudicatorIntervention") && !hero.HasBuff("kindredrnodeathbuff") && !hero.HasBuff("Undying Rage") && !hero.IsDead && !hero.IsZombie))
            {
                if (KsE && E.IsReady() && target.IsValidTarget(E.Range))
                {
                    if (target.Health + target.AllShield < Player.Instance.GetSpellDamage(target, SpellSlot.E))
                    {
                        E.Cast(target);
                    }
                }
