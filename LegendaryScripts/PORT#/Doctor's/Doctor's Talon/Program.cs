...
namespace Talon7
{
    internal class Program
    {
        ...
        static void OnLoadingComplete()
        {
            ...
            Chat.Print("Doctor's Talon PORTED By DEATHGODX!", Color.Orange);
            Bootstrap.Init(null);
        ...
        public static void ResetAttack(object e, OrbwalkerActionArgs args)
        {
            ...
            if (target != null)
            {
                ...
                if (HasQ && Q.IsReady() && Orbwalker.ActiveMode.HasFlag(OrbwalkerMode.Harass) && target.IsValidTarget(150) && Player.Instance.ManaPercent > ManaW)
                {
                    Q.Cast(target);
                    Orbwalker.ResetAutoAttackTimer();
                    Player.IssueOrder(GameObjectOrder.AttackUnit, target);
        ...
        public static void KillSteal()
        {
            ...
            foreach (var target in GameObjects.EnemyHeroes.Where(hero => hero.IsValidTarget(W.Range) && !hero.HasBuff("JudicatorIntervention") && !hero.HasBuff("kindredrnodeathbuff") && !hero.HasBuff("Undying Rage") && !hero.IsDead && !hero.IsZombie))
            {
                if (KsQ && Q.IsReady())
                {
                    if (target != null)
