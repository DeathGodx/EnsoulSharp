...
namespace XinZhao7
{
    static class Program
    {
        ...
        static void OnLoadingComplete()
        {
            ...
            Chat.Print("Doctor's Xinzhao Loaded! PORTEX by DEATHGODx", Color.Orange);
            Bootstrap.Init(null);
        ...
        private static void Combo()
        {
            ...
            if (target != null)
            {
                ...
                if (useW && W.IsReady() && target.IsValidTarget(800) && !target.IsDead && !target.IsZombie)
                {
                    W.Cast(target);
                }
        ...
        private static void Ultimate()
        {
            ...
            if (useR && !Player.Instance.InShop() && _Player.Position.CountEnemyHeroesInRange(R.Range) >= minR)
            {
                R.Cast();
            }

            if (useR2 && _Player.HealthPercent <= mauR && _Player.Position.CountEnemyHeroesInRange(R.Range) >= 1 && !Player.Instance.InShop())
            {
                R.Cast();
            }
        }
        ...
        private static void ResetAttack(object target, OrbwalkerActionArgs args)
        {
            ...
            if (target != null)
            {
                if (useriu && Orbwalker.ActiveMode.HasFlag(OrbwalkerMode.Combo) || Orbwalker.ActiveMode.HasFlag(OrbwalkerMode.LaneClear))
                {
        ...
        public static void Interupt(object sender, InterruptSpellArgs i)
        {
            ...
            if (!(sender is AIHeroClient) || Player.Instance.IsRecalling())
            {
                return;
            }
        ...
        private static void KillSteal()
        {
            ...
            foreach (var target in GameObjects.EnemyHeroes.Where(hero => hero.IsValidTarget(R.Range) && !hero.HasBuff("FioraW") && !hero.HasBuff("JudicatorIntervention") && !hero.HasBuff("kindredrnodeathbuff") && !hero.HasBuff("Undying Rage") && !hero.HasBuff("SpellShield") && !hero.HasBuff("NocturneShield") && !hero.IsDead && !hero.IsZombie))
            {
                if (KsE && E.IsReady() && target.IsValidTarget(E.Range))
                {
                    if (target.Health + target.Health + target.AllShield < Player.Instance.GetSpellDamage(target, SpellSlot.E))
                    {
                        E.Cast(target);
                    }
                }
                ...
                {
                    if (target.Health + target.Health + target.AllShield < Player.Instance.GetSpellDamage(target, SpellSlot.R))
                    {
                        R.Cast();
                    }
                }
