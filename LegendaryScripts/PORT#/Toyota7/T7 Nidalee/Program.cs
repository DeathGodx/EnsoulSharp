...
namespace T7_Nidalee
{
    class Program
    {
        ...
        {
            if (Player.Instance.CharacterName != ChampionName) { return; }

            Chat.Print("<font color='#0040FF'>T7</font><font color='#FF0505'> " + ChampionName + "</font> : Loaded! DEATHGODxPORTED(v" + Version + ")");
        ...
        private static void OnTick(EventArgs args)
        {
            ...
            if (flags.HasFlag(OrbwalkerMode.Harass) && myhero.ManaPercent > MenuSlider(harass, "HMIN")) Harass();

            if (flags.HasFlag(OrbwalkerMode.LaneClear) && myhero.ManaPercent > MenuSlider(laneclear, "LMIN")) Laneclear();

            if (flags.HasFlag(OrbwalkerMode.LaneClear) && myhero.ManaPercent > MenuSlider(jungleclear, "JMIN")) Jungleclear();

            if (flags.HasFlag(OrbwalkerMode.None) && check(misc, "W2FLEE") && myhero.Spellbook.GetSpell(SpellSlot.W).IsLearned)
            {
                if (IsCougar())
                {
                    DemSpells.W2.Cast(myhero.Position.Extend(Game.CursorPosRaw, DemSpells.W2.Range - 1.0f).ToVector2());
                }
                else
                {
                ...
                switch (IsHunted(target))
                {
                    case true:
                        if (target.IsValidTarget(DemSpells.W2E.Range))
                        {
                            ...
                            if (wpred.AoeTargetsHitCount >= MenuSlider(pred, "W2Pred")) DemSpells.W2E.Cast(wpred.CastPosition);
                        }
                        break;
                    case false:
                        if (target.IsValidTarget(DemSpells.W2.Range))
                        {
                            ...
                            if (wpred.AoeTargetsHitCount >= MenuSlider(pred, "W2Pred")) DemSpells.W2.Cast(wpred.CastPosition);
                        }
                        break;
                }
            }
        ...
        private static void Combo()
        {
            ...
            if (target != null)
            {
                if (IsCougar())
                {
                    ...
                    {
                        if (target.Health + target.AllShield <= DemSpells.E2.Range)
                        {
                ...
                else if (!IsCougar())
                {
                    if (check(combo, "CQ1") && DemSpells.Q1.CanCast(target))
                    {
                        ...
                        if (qpred.AoeTargetsHitCount >= MenuSlider(pred, "QPred")) DemSpells.Q1.Cast(qpred.CastPosition); return;
                    }
                    ...
                    if (check(combo, "CW1") && DemSpells.W1.CanCast(target))
                    {
                        ...
                        if (wpred.AoeTargetsHitCount >= MenuSlider(pred, "W1Pred")) DemSpells.W1.Cast(wpred.CastPosition); return;
                    }
                }
        ...
        private static void Laneclear()
        {
            ...
            if (minions != null)
            {
                if (!IsCougar())
                {
                    if (check(laneclear, "LQ1") && DemSpells.Q1.IsReady())
                    {
                        foreach (var minion in minions.Where(x => x.IsValidTarget(DemSpells.Q1.Range) && x.Health > 40).OrderBy(x => x.Distance(myhero.Position)))
                        {
                            ...
                            if ((comb(laneclear, "LQ1MODE") == 0 && !minion.Name.ToLower().Contains("siege") || minion.Name.ToLower().Contains("super"))) continue;

                            if (qpred.AoeTargetsHitCount >= MenuSlider(pred, "QPred")) DemSpells.Q1.Cast(qpred.CastPosition);
                        }
                    }
                }
                else if (IsCougar())
                {
                    ...
                    {
                        DemSpells.W2.CastOnBestTarget(MenuSlider(laneclear, "LW2MIN"));
                    }
                    ...
                    if (check(laneclear, "LE2") && DemSpells.E2.IsReady())
                    {
                        ...
                        {
                            if (comb(laneclear, "LE2MODE") == 0 && !minion.Name.ToLower().Contains("siege") || minion.Name.ToLower().Contains("super")) continue;

                            var epred = DemSpells.E2.GetPrediction(minion);

                            if (epred.AoeTargetsHitCount >= MenuSlider(pred, "EPred")) DemSpells.E2.Cast(epred.CastPosition);
                        }
                    }
                }
        ...
        {
            var Monsters = GameObjects.EnemyMinions.Where(e => e.IsValidTarget(1600) && e.IsMinion())
                           .Cast<AIBaseClient>().ToList();
            ...
            if (Monsters != null)
            {
                ...
                else if (IsCougar())
                {
                    ...
                    {
                        if (Monsters.Where(x => x.IsValidTarget(DemSpells.Q2.Range) && x.Health > 30).Count() >= MenuSlider(jungleclear, "JQ2MIN")) DemSpells.Q2.Cast();
                    }
                ...
                if (check(jungleclear, "JAUTOR") && DemSpells.R.IsReady())
                {
                    ...
                    if (!IsCougar())
                    {
                        if (((W2Ready && Monsters.Where(x => x.IsValidTarget(DemSpells.W2.Range)).Count() >= 1) ||
        ...
        public static void DatMenu()
        {
            ...
            menu.Add(new MenuSeparator("Welcome to T7 " + ChampionName + " And Thank You For Using!", "Welcome to T7 " + ChampionName + " And Thank You For Using!"));
            ...
            menu.Add(new MenuSeparator("Please Report Any Bugs And If You Have Any Requests Feel Free To PM Me <3", "Please Report Any Bugs And If You Have Any Requests Feel Free To PM Me <3"));
